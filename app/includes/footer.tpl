<footer class="footer" role="content" aria-label="footer" ng-controller='footerCtrl' >
<div class="container">
  <article class="block__wrap" >

    <section class="footer--left">
      <a href="/"><img src="images/maincoin.png" height="45px"  alt="Ether Wallet" class="footer--logo"/> <span>&copy; 2018 MainCoin wallet </span> </a>
    <!--
      <p>
        <span translate="FOOTER_1">
          Free, open-source, client-side interface for generating Ethereum wallets &amp; more. Interact with the Ethereum blockchain easily &amp; securely. Double-check the URL ( myetherwallet.com ) before unlocking your wallet.
        </span>
      </p> -->

      

     <!-- <p ng-show="showBlocks">
        Latest Block#: {{currentBlockNumber}}
      </p> -->
     
      <p>
        <br />
        MAINCOIN MONEY LTD<br>
        &copy; ALL RIGHTS RESERVED
      </p>

    </section>
    <section class="footer--cent">
      <h4 class="widget_title">Maincoin Exchange</h4>
      
      <article class="footer__link-row" >
        <div class="footer__link-column">
          <a target="_blank" href="https://exmo.me" title="EXMO Exchange">Exmo</a><br>
          <a target="_blank" href="https://p2pb2b.io" title="P2PB2B">P2PB2B</a><br>
          <a target="_blank" href="https://livecoin.net" title="Livecoin">Livecoin</a><br>
          <a target="_blank" href="https://www.bikicoin.pro/index" title="Bikicoin">Bikicoin</a><br>
          <a target="_blank" href="https://exrates.me/dashboard" title="Exrates Exchange">Exrates</a><br>
        </div>
        <div class="footer__link-column">
          <a target="_blank" href="https://deex.exchange/market/MAINCOIN_DEEX.ETH" title="DEEX Exchange">DEEX</a><br>
          <a target="_blank" href="https://btc-alpha.com/exchange/MNC_USD" title="BTC Alpha">BTC Alpha</a><br>
          <a target="_blank" href="https://bitfex.trade/ru/MNC/BTC" title="BITFEX">BITFEX</a><br>
          <a target="_blank" href="https://alterdice.com" title="Alterdice">Alterdice</a><br>
          <a target="_blank" href="https://localtrade.cc" title="Localtrade">Localtrade</a><br>
        </div>
        <div class="footer__link-column">
          <a target="_blank" href="https://www.bw.com/" title="BW Exchange">BW</a><br>
          <a target="_blank" href="https://www.fubt.com/#/home" title="Fubt">Fubt</a><br>
          <a target="_blank" href="https://www.stex.com/" title="Stex">Stex</a><br>
          <a target="_blank" href="https://www.dobiexchange.com/" title="DOBI">DOBI</a><br>
          <a target="_blank" href="https://www.bitker.com/" title="Bitker">Bitker</a><br>
        </div>
        <div class="footer__link-column">
          <a target="_blank" href="https://bit.ly/2TPd1kA" title="Latoken Exchange">Latoken</a><br>
          <a target="_blank" href="https://mercatox.com/" title="Merkatox">Merkatox</a><br>
          <a target="_blank" href="https://sistemkoin.com/" title="Sistemkoin">Sistemkoin</a><br>
        </div>
      </article>
    </section> 
   <!--  <section class="footer--righ">
    <div class="footer__pill-wrap">
      <h3 class="widget_title">КОНТАКТНАЯ ИНФОРМАЦИЯ</h3>
      <div class="textwidget">
        <span id="span_e4d5_5">
        <span class="nz-icon none small icon-home animate-false" id="span_e4d5_6"></span>41 Tabernacle Street EC2A 4AA
        <br>
        <span class="nz-icon none small icon-location animate-false" id="span_e4d5_7"></span>London, United Kingdom
        <br>
        <span class="nz-icon none small icon-iphone animate-false" id="span_e4d5_8"></span>Tel. 8 (800) 600 08 77
        <br>
        <span class="nz-icon none small icon-envelope animate-false" id="span_e4d5_10"></span>E-mail: support@avtomain.pro
        <br>
        <span class="nz-icon none small icon-cloud2 animate-false" id="span_e4d5_11"></span>Web: avtomain.pro
        </span>
    </div>
    </div>
    </section> -->
  </article>
</div>


</footer>



</main>
</body>
</html>
