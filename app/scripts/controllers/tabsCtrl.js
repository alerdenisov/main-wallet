...
var tabsCtrl = function($scope, globalService, $translate, $sce) {
  ...
  $scope.addCustomNodeToList = function(nodeInfo) {
    var tempObj = null;
    if (nodeInfo.options == "eth")
      tempObj = JSON.parse(JSON.stringify(nodes.nodeList.eth_ethscan));
    else if (nodeInfo.options == "etc")
      tempObj = JSON.parse(JSON.stringify(nodes.nodeList.etc_epool));
  ...
  $scope.removeNodeFromLocal = function(localNodeName) {
    ...
    localNodes = !localNodes ? [] : JSON.parse(localNodes);
    for (var i = 0; i < localNodes.length; i++) {
      if (localNodes[i].name + ":" + localNodes[i].options == localNodeName)
  ...
  $scope.setErrorMsgLanguage = function() {
    ...
    for (var i = 0; i < globalFuncs.phishingWarning.length; i++)
      $scope.setLanguageVal(
        "PHISHING_Warning_" + (i + 1),
        "phishingWarning",
        i
      );
  };
  ...
  $scope.setLanguageFromStorage = function() {
    ...
    if (lang == null) lang = '{"key":"ru","value":"Русский"}';
